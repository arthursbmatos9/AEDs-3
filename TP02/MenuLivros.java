...
public class MenuLivros {
  ...
  public MenuLivros() {
    try {
      arqLivros = new ArquivoLivros();//instanciação
  ...
  public void menu() {
    ...
    do {
      ...
      System.out.println("\n> Início > Livros");
      System.out.println("\n1) Inclui livro");
      System.out.println("2) Busca livro pelo ISBN");
    ...
    do{
      ...
      try { 
        ...
        if (lista == null || lista.length == 0 ) { 
          System.out.println("Sem livros com esse titulo");
          return;
        }
        else imprimeLivros(lista);
      } catch (Exception e) {
        e.printStackTrace();
      }
    }while(titulo.length()==0);//busca em branco
  
  }

  public void imprimeLivros(Livro[] lista) throws Exception { //metodo para imprimir uma lista de livros
    for(int i = 0; i < lista.length; i++) // percorre o array de livros
    {
      mostraLivro(lista[i]); // mostra os livros
    }
  }
