...
public class Arquivo<T extends Registro> {
  ...
  protected ArrayList<Long> arrayEnderecos = new ArrayList<>();

  public Arquivo(String na, Constructor<T> c) throws Exception {
    this.nomeEntidade = na;
    this.construtor = c;

    arquivo = new RandomAccessFile(na + ".db", "rw");
    ...
    indiceDireto = new HashExtensivel<>(ParIDEndereco.class.getConstructor(),
        3,
        na + ".hash_d.db", na + ".hash_c.db");
  }

  private long findLapidePosition(short tamNovo) throws Exception {
    short tamAntigo;
    long novoEndereco;

    // itera sobre o arrayList que contem os enderecos dos excluidos
    for (int i = 0; i < arrayEnderecos.size(); i++) {
      novoEndereco = arrayEnderecos.get(i);
      arquivo.seek(novoEndereco + 1); // vai pra posicao do tamanho do registro
      tamAntigo = arquivo.readShort();

      // aceita-se um desperdicio de apenas 10% do tamanho do registro original
      if (tamNovo <= tamAntigo && tamNovo >= tamAntigo * 0.9) {
        arrayEnderecos.remove(i);
        return novoEndereco;
      }
    }

    return -1; // Retorna -1 se não encontrar espaco vazio no arquivo, entao, inserir no final
  }
  ...
  public int create(T obj) throws Exception {
    ...
    long endereco = findLapidePosition(tam);

    // se não puder reaproveitar espacos, mandar o ponteiro pro final,
    // caso contrario, mandar o ponteiro para o inicio do registro excluido
    if (endereco == -1)
      arquivo.seek(arquivo.length());
    else
      arquivo.seek(endereco);

    // inserindo no indice e no arquivo de dados
  ...
  public boolean delete(int id) throws Exception {
    ...
    if (endereco != -1) {
      ...
      arrayEnderecos.add(endereco); // inserindo o endereco do registro deletado no array
      return true;
    }
  ...
  public boolean update(T novoObj) throws Exception {
    ...
    if (endereco != -1) {
      arquivo.seek(endereco + 1); // indo ate a posicao do registro

      // lendo e salvando o registro antes de ser atualizado
      tam = arquivo.readShort();
      ba = new byte[tam];
      arquivo.read(ba);
      obj.fromByteArray(ba);

      // pegando tamanho do registro atualizado
      ba2 = novoObj.toByteArray();
      tam2 = (short) ba2.length;

      // registro atualizado sendo menor em ate 10% ou igual, pode manter o endereco
      if (tam2 <= tam && tam2 >= tam * 0.9) {
        arquivo.seek(endereco + 1 + 2);
        arquivo.write(ba2);
      }

      // caso contrario, deletar o registro e escrever o atualizado outro lugar
      else {
        delete(novoObj.getID());

        endereco = findLapidePosition(tam2);
        arquivo.seek(endereco);
        arquivo.writeByte(' ');
  ...
  public void reorganizar() throws Exception {
    ...
    indiceDireto = new HashExtensivel<>(ParIDEndereco.class.getConstructor(),
        3,
        this.nomeEntidade + ".hash_d.db",
        this.nomeEntidade + ".hash_c.db");
